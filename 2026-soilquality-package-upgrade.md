...
- [ ] **0.2 Establish a regression baseline.** Run the existing test suite; record pass/fail per
  file. Nine test files already exist (~2 070 lines) covering ahp, data-handling, interactive,
  pca-mds, property-sets, scoring, scoring-constructors, sqi-compute, visualization. If coverage of
  `compute_sqi_df()` (the engine, not just the wrapper) is thin, add a golden-output test on
...
- [ ] **3.3 Implement `sqi_compare()`** — run the same data through ≥ 2 recipes (e.g. linear vs
