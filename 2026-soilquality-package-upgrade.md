...
delivered_through: phase-4 (v1.4.0, merged to master)
---
...
- **Out of scope (non-goals):** spatial prediction (that is [[skills-forge/soil-fertility-mapping/SKILL|soil-fertility-mapping]]'s
  ...
  the existing API.
- ~~**First slice:** Phase 1 + Phase 2.~~ **Superseded twice — see below.**
  - *Revised 2026-08-08 (strategic):* open with **Phase 2 + Phase 3**, the two strongest
    differentiators against the CRAN packages. Phase 1 drops to "when convenient" because
    `SQIpro::score_custom()` already lets a user supply the sigmoid. See the strategic section.
  - *What actually shipped:* **Phases 0 through 4, all merged to `master`** (v1.4.0), plus the
    `soil_structured` dataset. Phase 1 was built before the strategic analysis existed. It is done
    and green; no reason to unpick it, but it explains why the delivered order does not match the
    revised recommendation. **Phases 5, 6 and 7 remain**, and 5 and 6 are both differentiators.
...
- [x] **3.3 Implement `sqi_stability()`** ✅ Done — run the same data through ≥ 2 recipes (e.g. linear vs
...
- [x] **⚠️ 4.7 NEW — `soil_data` cannot exercise this route.** The shipped example data is
  ...
  its own test so nobody concludes the method is broken.
  ✅ **RESOLVED — `soil_structured` added** (120 samples, 16 columns, generated from three latent
  gradients in `data-raw/create_soil_structured.R`). `soil_data` and `soil_ucayali` are left
  untouched. **40** indicator pairs reach |ρ| ≥ 0.6 against **1** in `soil_data`, with the standard
  relationships holding: texture compositional to 100, `SOC = OM/1.724` (ρ = 0.99), `N = SOC/(C:N)`,
  CEC from clay + organic colloids, `OM ~ BD` = −0.85, `Ca ~ EC` = 0.92. Use it for Phase 5.
  ⚠️ It also exposed a **bug in Task 4.2's screen**: the redundancy screen ran across the whole
  selected set instead of within each module, so every module's representative was discarded as
  redundant with the single most central indicator — collapsing the very partition the module step
  had just built. Fixed; the MDS went from 1 indicator to 2 on this data.
  ⚠️ And it usefully sets up Phase 5: the base-status module (`pH`, `Ca`, `Mg`, `EC`) is internally
  coherent but peripheral, so the centrality filter discards it entirely. Selecting **within
  functional groups** rather than across the whole pool is the fix, which is exactly Phase 5.
...
- **2026-08-08** — **Phases 0–4 merged to `master` as v1.4.0**, in four pull requests, each green on
  the full CI matrix (macOS, Windows, Ubuntu devel/release/oldrel-1) plus the pkgdown build.
  Phase 1 (`score_sigmoid()` + `sigmoid_scoring()`), Phase 2 (`sqi_area()`, `method = "area"`),
  Phase 3 (`sqi_validate()`, `sqi_stability()`, `plot_sqi_validation()`), Phase 4
  (`na_select_mds()`, `mds_consensus()`, `select = "network"`), plus the `soil_structured` dataset
  and the migration to `testthat` edition 3.
- **2026-08-08** — ⚠️ **CI caught two things local verification could not.**
  1. **`R CMD check` does not validate the pkgdown reference index.** The site uses an explicit
     index and aborts on any exported function missing from it. Four branches reported 0/0/0
     locally while the site build was broken. **Add every new export to `_pkgdown.yml` in the same
     commit that exports it.**
  2. **"Exactly zero" was platform-dependent.** The Phase 4 finding that eigenvector centrality is
     *bit-exactly* 0 outside the dominant component holds under the reference BLAS and fails under
     macOS Accelerate, which returns ≈1e-17. The substance stands — neither passes a usable
     `centrality_min` — but the claim was stronger than the arithmetic supports. Assertions on
     floating-point identities need a tolerance, and prose needs to match.
- **2026-08-08** — ⚠️ **This document was briefly clobbered and has been repaired.** A commit merged
  the strategic section on top of a pre-correction copy, deleting 247 lines: every Phase 0–4
  correction and 20 completed-task marks. Restored from `3022e6e~1` with the strategic section
